...
RC Table::set_value_to_record(char *record_data, const Value &value, const FieldMeta *field)
{
  ...
  if (field->type() == AttrType::CHARS) {
    ...
    if (data_len != copy_len) {
      LOG_WARN("vector dimension mismatch. table=%s, field=%s, field_dim=%d bytes, value_dim=%d bytes",
               table_meta_.name(), field->name(), static_cast<int>(copy_len), static_cast<int>(data_len));
      return RC::INVALID_ARGUMENT;
    }
  }
...
RC Table::add_column(const FieldMeta &field_meta)
{
  return engine_->add_column(field_meta);
}
