...
RC DefaultConditionFilter::init(Table &table, const ConditionSqlNode &condition)
{
  ...
  // VECTORS type only supports equality comparison (= and !=)
  if (type_left == AttrType::VECTORS) {
    if (condition.comp != EQUAL_TO && condition.comp != NOT_EQUAL) {
      LOG_WARN("VECTOR type only supports equality comparison, got comp_op=%d", condition.comp);
      return RC::SCHEMA_FIELD_TYPE_MISMATCH;
    }
  }
