...
void Value::set_data(char *data, int length)
{
  switch (attr_type_) {
    ...
    case AttrType::VECTORS: {
      set_vector(reinterpret_cast<const float *>(data), length / static_cast<int>(sizeof(float)));
    } break;
    case AttrType::INTS: {
...
void Value::set_vector(const float *data, int dim)
{
  ...
  own_data_ = true;
  length_   = dim * static_cast<int>(sizeof(float));
...
void Value::set_value(const Value &value)
{
  switch (value.attr_type_) {
    ...
    case AttrType::VECTORS: {
      set_vector(reinterpret_cast<const float *>(value.data()),
