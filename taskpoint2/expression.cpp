...
FunctionExpr::FunctionExpr(Type func_type, unique_ptr<Expression> arg1,
    unique_ptr<Expression> arg2, unique_ptr<Expression> arg3)
    : func_type_(func_type)
{
  args_.emplace_back(std::move(arg1));
  if (arg2) {
    args_.emplace_back(std::move(arg2));
  }
  if (arg3) {
    args_.emplace_back(std::move(arg3));
  }
}
...
{
  if (args_.size() == 1) {
    return make_unique<FunctionExpr>(func_type_, args_[0]->copy());
  } else if (args_.size() == 2) {
    return make_unique<FunctionExpr>(func_type_, args_[0]->copy(), args_[1]->copy());
  } else {
    return make_unique<FunctionExpr>(func_type_, args_[0]->copy(), args_[1]->copy(), args_[2]->copy());
  }
}
...
{
  switch (func_type_) {
    case Type::STRING_TO_VECTOR: return -1;
    case Type::VECTOR_TO_STRING: return -1;
...
{
  // Try constant folding: if all args are constant, evaluate
  ...
  switch (func_type_) {
    case Type::STRING_TO_VECTOR: {
      string str = arg_values[0].get_string();
      return DataType::type_instance(AttrType::VECTORS)->set_value_from_str(value, str);
    }
    case Type::VECTOR_TO_STRING: {
      string result;
      RC rc = DataType::type_instance(AttrType::VECTORS)->to_string(arg_values[0], result);
      if (OB_SUCC(rc))
        value.set_string(result.c_str());
      return rc;
    }
    case Type::DISTANCE: {
      string method = arg_values[2].get_string();
      return compute_distance(arg_values[0], arg_values[1], method, value);
    }
  }
...
RC FunctionExpr::get_value(const Tuple &tuple, Value &value) const
{
  ...
  switch (func_type_) {
    case Type::STRING_TO_VECTOR: {
      Value str_val;
      rc = args_[0]->get_value(tuple, str_val);
      if (OB_FAIL(rc)) {
        LOG_WARN("STRING_TO_VECTOR: failed to get argument value");
        return rc;
      }
      string str = str_val.get_string();
      rc = DataType::type_instance(AttrType::VECTORS)->set_value_from_str(value, str);
      if (OB_FAIL(rc)) {
        LOG_WARN("STRING_TO_VECTOR: failed to parse vector string: %s", str.c_str());
      }
      return rc;
    }
    ...
    case Type::VECTOR_TO_STRING: {
      Value vec_val;
      rc = args_[0]->get_value(tuple, vec_val);
      if (OB_FAIL(rc)) {
        LOG_WARN("VECTOR_TO_STRING: failed to get argument value");
        return rc;
      }
      if (vec_val.attr_type() != AttrType::VECTORS) {
        LOG_WARN("VECTOR_TO_STRING: argument is not VECTOR type");
        return RC::INVALID_ARGUMENT;
      }
      string result;
      rc = DataType::type_instance(AttrType::VECTORS)->to_string(vec_val, result);
      if (OB_FAIL(rc)) {
        LOG_WARN("VECTOR_TO_STRING: failed to convert vector to string");
        return rc;
      }
      value.set_string(result.c_str());
      return RC::SUCCESS;
    ...
    case Type::DISTANCE: {
      Value v1, v2, method_val;
      rc = args_[0]->get_value(tuple, v1);
      if (OB_FAIL(rc))
        return rc;
      rc = args_[1]->get_value(tuple, v2);
      if (OB_FAIL(rc))
        return rc;
      rc = args_[2]->get_value(tuple, method_val);
      if (OB_FAIL(rc))
        return rc;

      string method = method_val.get_string();
      return compute_distance(v1, v2, method, value);
    }
  }
...
RC FunctionExpr::compute_distance(const Value &v1, const Value &v2, const string &method, Value &result) const
{
  float dist = 0.0f;
  RC    rc   = RC::SUCCESS;
  ...
  if (method_upper == "EUCLIDEAN" || method_upper == "L2") {
    rc = VectorType::euclidean_distance(v1, v2, dist);
  } else if (method_upper == "DOT") {
    rc = VectorType::dot_product(v1, v2, dist);
  } else if (method_upper == "COSINE") {
    rc = VectorType::cosine_distance(v1, v2, dist);
  } else {
    LOG_WARN("DISTANCE: unsupported method '%s'. Supported: COSINE, DOT, EUCLIDEAN/L2", method.c_str());
    return RC::INVALID_ARGUMENT;
  }
...
{
  if (0 == strcasecmp(type_str, "string_to_vector")) {
