...
FunctionExpr *create_function_expression(FunctionExpr::Type func_type,
                                         Expression *arg1,
                                         Expression *arg2,
                                         Expression *arg3,
                                         const char *sql_string,
                                         YYLTYPE *llocp)
{
  FunctionExpr *expr = new FunctionExpr(func_type,
      unique_ptr<Expression>(arg1),
      unique_ptr<Expression>(arg2),
...
alter_table_stmt:   /*alter table add column 语句的语法解析树*/
    ...
    ID type LBRACE number RBRACE
    {
      ...
      if ($$->type == AttrType::VECTORS) {
        ...
        if (dim <= 0 || dim > VECTOR_MAX_DIM) {
          yyerror(&@4, sql_string, sql_result, scanner,
            "invalid vector dimension, must be in range [1, 16383]");
          YYERROR;
        }
...
value:
    ...
    | STRING_TO_VECTOR_T LBRACE SSS RBRACE {
      ...
      // parse JSON array [num1,num2,...]
      vector<float> vec;
      const char *p = tmp;
      while (*p == ' ') p++;
      if (*p != '[') {
        yyerror(&@3, sql_string, sql_result, scanner, "STRING_TO_VECTOR expects a JSON array starting with '['");
        free(tmp);
        YYERROR;
      }
      p++;
      while (*p) {
        ...
        if (end == p) {
          yyerror(&@3, sql_string, sql_result, scanner, "invalid number in STRING_TO_VECTOR argument");
          free(tmp);
          YYERROR;
        }
        vec.push_back(val);
        ...
        if (*p == ',') p++;
          ...
          yyerror(&@3, sql_string, sql_result, scanner, "expected ',' or ']' in STRING_TO_VECTOR argument");
          free(tmp);
          YYERROR;
        }
      }
      if (*p != ']') {
        yyerror(&@3, sql_string, sql_result, scanner, "expected closing ']' in STRING_TO_VECTOR argument");
        free(tmp);
        YYERROR;
      }
      if (vec.empty()) {
        yyerror(&@3, sql_string, sql_result, scanner, "vector must have at least one element");
        free(tmp);
        YYERROR;
      }
      $$->set_vector(vec.data(), static_cast<int>(vec.size()));
      free(tmp);
    }
    ;
storage_format:
...
function_expression:
    STRING_TO_VECTOR_T LBRACE expression RBRACE {
      $$ = create_function_expression(FunctionExpr::Type::STRING_TO_VECTOR,
                                      $3, nullptr, nullptr, sql_string, &@$);
    }
    | VECTOR_TO_STRING_T LBRACE expression RBRACE {
      $$ = create_function_expression(FunctionExpr::Type::VECTOR_TO_STRING,
                                      $3, nullptr, nullptr, sql_string, &@$);
    }
    | DISTANCE LBRACE expression COMMA expression COMMA expression RBRACE {
      $$ = create_function_expression(FunctionExpr::Type::DISTANCE,
                                      $3, $5, $7, sql_string, &@$);
    }
    ;
