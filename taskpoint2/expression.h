 ...
 * @brief 自定义函数表达式
 * @ingroup Expression
 * @details 支持 STRING_TO_VECTOR, VECTOR_TO_STRING, DISTANCE 等标量函数
 */
...
public:
  FunctionExpr(Type func_type, unique_ptr<Expression> arg1,
               unique_ptr<Expression> arg2 = nullptr,
               unique_ptr<Expression> arg3 = nullptr);
  ...
  Type function_type() const { return func_type_; }

  const vector<unique_ptr<Expression>> &args() const { return args_; }

public:
  static RC type_from_string(const char *type_str, Type &type);

private:
  RC compute_distance(const Value &v1, const Value &v2, const string &method, Value &result) const;
