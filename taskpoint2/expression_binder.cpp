    ...
    unique_ptr<Expression> &expr, vector<unique_ptr<Expression>> &bound_expressions)
{
  ...
  // Recursively bind each child expression
  const auto &args = func_expr->args();
  for (size_t i = 0; i < args.size(); i++) {
    // Need to cast away constness to modify the args through const ref
    auto &mutable_args = const_cast<vector<unique_ptr<Expression>> &>(args);
    unique_ptr<Expression> &child_expr = mutable_args[i];
    ...
    if (child_bound_expressions.size() != 1) {
      LOG_WARN("invalid children number of function expression arg %zu: %d", i, child_bound_expressions.size());
      return RC::INVALID_ARGUMENT;
    }
